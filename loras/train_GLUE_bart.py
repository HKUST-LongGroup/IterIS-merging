...
from transformers import T5Tokenizer, BartForConditionalGeneration, Trainer, TrainingArguments, DataCollatorForSeq2Seq
...
def prompt_text(input1, task_name, input2=None):
    ...
    if task_name == 'cola':
        ...
        return f"""Does {input1} imply {input2}? Please answer yes or no."""
    elif task_name == 'wnli':
        return f"""
...
def find_label(output_ids):
    for item in output_ids:
        if item in [10932, 2362, 31501]:
            return item
    return -100
...
def compute_metrics(eval_pred):
    ...
    labels = labels[:, 1]
    preds = [find_label(pred) for pred in preds]
...
def main():
    ...
    parser.add_argument(
        '--config',
        type=str,
        default="config/GLUE-bart-lora-train-config/MNLI-lora-train.yaml",
    ...
    model = BartForConditionalGeneration.from_pretrained("facebook/bart-base", max_length=256)
